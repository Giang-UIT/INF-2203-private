      ...
      int       slowdown)
{
    ...
    byte = (void *) addr;
    cpos = (addr - addr_min) / addrs_per_cell;
    for (; byte < endbyte; cpos++) {
        ...
        int  r         = cpos / screen_cols;
        int  c         = cpos % screen_cols;
        draw_char(r, c, indicator, GREY_ON_BLACK);
        delay_loop(slowdown);
    }
    ...
    for (; byte < endbyte; cpos++) {
        ...
        int  r         = cpos / screen_cols;
        int  c         = cpos % screen_cols;
        draw_char(r, c, indicator, DARK_RED);
        delay_loop(slowdown);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    fprintf_erasedisplay(stdout, ED_SCREEN);
    for (int i = 1; i < argc; i++) {
        if (argv[i][0] != '-') {
            uintptr_t addr;
            size_t    size;
